...
export default function MarketingPage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f7f7f3] text-[#17211b]">
      <header className="sticky top-0 z-30 border-b border-[#dfe5dc] bg-[#f7f7f3]/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          <Link href="/marketing" className="flex items-center gap-3" aria-label="Natural Intellects home">
            ...
            <span className="hidden text-sm font-bold tracking-[0.1em] text-[#173b2b] sm:block">NATURAL INTELLECTS</span>
          </Link>
          <nav className="hidden items-center gap-7 text-sm text-[#526158] lg:flex" aria-label="Primary navigation">
            {[['features', 'Features'], ['how-it-works', 'How it works'], ['pricing', 'Pricing'], ['security', 'Security']].map(([id, label]) => <a key={id} href={`#${id}`} className={`border-b-2 py-2 transition-colors ${activeSection === id ? 'border-[#b2761b] text-[#173b2b]' : 'border-transparent hover:text-[#173b2b]'}`}>{label}</a>)}<a href="https://nisms.vercel.app" target="_blank" rel="noreferrer" className="border-b-2 border-transparent py-2 transition-colors hover:text-[#173b2b]">NISMS</a>
          </nav>
        ...
        {menuOpen && <nav className="flex flex-col gap-2 border-t border-[#dfe5dc] px-5 py-4 text-sm lg:hidden" aria-label="Mobile navigation">{[['features', 'Features'], ['how-it-works', 'How it works'], ['pricing', 'Pricing'], ['security', 'Security']].map(([id, label]) => <a key={id} href={`#${id}`} className="rounded-md px-2 py-3 focus:outline-none focus:ring-2 focus:ring-[#b2761b]" onClick={() => setMenuOpen(false)}>{label}</a>)}<a href="https://nisms.vercel.app" target="_blank" rel="noreferrer" className="rounded-md px-2 py-3 font-semibold text-[#173b2b]" onClick={() => setMenuOpen(false)}>NISMS</a><Link href="/login" className="rounded-md px-2 py-3">Log in</Link><Link href="/start-free-trial" className="rounded-md px-2 py-3 font-semibold text-[#173b2b]">Start free trial <ArrowRight className="ml-1 inline h-4 w-4" /></Link></nav>}
      </header>
      ...
      <section className="mx-auto grid max-w-7xl items-center gap-12 px-5 pb-20 pt-16 lg:grid-cols-[1.02fr_.98fr] lg:px-8 lg:pb-28 lg:pt-24">
        ...
        <div className="border border-[#cbd6cb] bg-[#173b2b] p-4 shadow-2xl shadow-[#173b2b]/15 sm:p-6"><div className="bg-[#f7f7f3] p-5 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#829086]">Manager view</p><h2 className="mt-2 text-xl font-semibold text-[#173b2b]">A practical view of reporting</h2></div><span className="border border-[#c8d2c7] px-3 py-1 text-xs font-medium text-[#526158]">Product preview</span></div><div className="mt-8 border-y border-[#dfe5dc] py-8"><p className="text-sm font-semibold text-[#173b2b]">Bring your organization&apos;s data into focus.</p><p className="mt-2 max-w-md text-sm leading-6 text-[#65746a]">Once connected, managers can see reporting health, recent activity, and monthly summaries in one place.</p><div className="mt-6 flex flex-wrap gap-2 text-xs font-semibold text-[#356247]"><span className="bg-[#e8eee5] px-3 py-2">Daily submissions</span><span className="bg-[#e8eee5] px-3 py-2">Team activity</span><span className="bg-[#e8eee5] px-3 py-2">Monthly insight</span></div></div><div className="mt-5 flex items-center gap-2 text-sm text-[#526158]"><Check className="h-4 w-4 text-[#356247]" /> No sample metrics—your workspace stays grounded in your data.</div></div></div>
      </section>

      <section id="features" className="scroll-mt-24 border-y border-[#dfe5dc] bg-[#edf1eb] px-5 py-20 lg:px-8"><div className="mx-auto max-w-7xl"><div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.22em] text-[#b2761b]">A calmer operating rhythm</p><h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-[#173b2b] sm:text-5xl">The reporting system people can actually keep up with.</h2></div><div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-[#d2ddd2] bg-[#d2ddd2] sm:grid-cols-2 lg:grid-cols-3">{features.map(({ icon: Icon, title, text }) => <article key={title} className="bg-[#edf1eb] p-7 transition-colors hover:bg-[#f7f7f3]"><Icon className="h-5 w-5 text-[#b2761b]" /><h3 className="mt-8 text-lg font-semibold text-[#173b2b]">{title}</h3><p className="mt-3 text-sm leading-6 text-[#65746a]">{text}</p></article>)}</div></div></section>

      <section id="how-it-works" className="scroll-mt-24 mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28"><div className="grid gap-12 lg:grid-cols-[.7fr_1.3fr]"><div><p className="text-xs font-bold uppercase tracking-[0.22em] text-[#b2761b]">One system, three useful views</p><h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-[#173b2b]">The right context for every role.</h2><p className="mt-5 leading-7 text-[#65746a]">Natural Intellects connects the daily note to the management decision without adding another maze of admin.</p><div className="mt-8 flex flex-wrap gap-2" role="tablist" aria-label="Product perspectives">{demos.map((demo, index) => <button key={demo.label} type="button" role="tab" aria-selected={activeDemo === index} onClick={() => setActiveDemo(index)} className={`rounded-full border px-4 py-2 text-sm font-semibold transition-colors ${activeDemo === index ? 'border-[#173b2b] bg-[#173b2b] text-[#f7f7f3]' : 'border-[#c8d2c7] text-[#526158] hover:bg-[#edf1eb]'}`}>{demo.label}</button>)}</div></div><div role="tabpanel" className="rounded-2xl border border-[#dfe5dc] bg-[#fbfcf8] p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#b2761b]">{demos[activeDemo].label}</p><h3 className="mt-4 text-3xl font-semibold tracking-[-0.04em] text-[#173b2b]">{demos[activeDemo].title}</h3><p className="mt-4 max-w-xl leading-7 text-[#65746a]">{demos[activeDemo].text}</p><ul className="mt-8 grid gap-3 sm:grid-cols-3">{demos[activeDemo].items.map((item) => <li key={item} className="rounded-xl bg-[#edf1eb] p-4 text-sm font-semibold text-[#304237]"><Check className="mb-4 h-4 w-4 text-[#b2761b]" />{item}</li>)}</ul></div></div><div className="mt-12 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{['Create your organization', 'Add employees and teams', 'Capture daily activity', 'Monitor reporting health', 'Generate monthly insight', 'Review, share, and export'].map((step, index) => <div key={step} className="flex items-center gap-4 rounded-xl border border-[#dfe5dc] p-4"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#173b2b] text-sm font-semibold text-[#f7f7f3]">{index + 1}</span><span className="text-sm font-semibold text-[#304237]">{step}</span></div>)}</div></section>

      <section id="security" className="scroll-mt-24 bg-[#173b2b] px-5 py-20 text-[#f7f7f3] lg:px-8 lg:py-24"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_1fr] lg:items-center"><div><p className="text-xs font-bold uppercase tracking-[0.22em] text-[#e9b44c]">Built with care</p><h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] sm:text-5xl">Your workforce data deserves a considered home.</h2><p className="mt-5 max-w-xl leading-7 text-[#c4d0c5]">Tenant-aware access, role-based permissions, secure authentication, and auditable activity are foundational—not add-ons.</p></div><div className="grid gap-3 sm:grid-cols-2">{[['Tenant isolation', 'Customer data is scoped server-side to its organization.'], ['Role-based access', 'Permissions follow responsibility, not guesswork.'], ['Audit logging', 'Important actions leave a clear, reviewable trail.'], ['Responsible exports', 'Reports respect ownership and organization boundaries.']].map(([title, text]) => <div key={title} className="rounded-xl border border-[#3d5c49] p-5"><ShieldCheck className="h-5 w-5 text-[#e9b44c]" /><h3 className="mt-5 font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-[#b6c5b8]">{text}</p></div>)}</div></div></section>

      <section id="pricing" className="scroll-mt-24 mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28"><div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.22em] text-[#b2761b]">Simple starting points</p><h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-[#173b2b]">Choose the shape that fits your team.</h2></div><div className="flex items-center gap-3 rounded-full border border-[#dfe5dc] bg-[#edf1eb] p-1 text-sm"><button type="button" aria-pressed={!annual} onClick={() => setAnnual(false)} className={`rounded-full px-4 py-2 font-semibold ${!annual ? 'bg-[#173b2b] text-[#f7f7f3]' : 'text-[#65746a]'}`}>Monthly</button><button type="button" aria-pressed={annual} onClick={() => setAnnual(true)} className={`rounded-full px-4 py-2 font-semibold ${annual ? 'bg-[#173b2b] text-[#f7f7f3]' : 'text-[#65746a]'}`}>Annual <span className="text-[#b2761b]">Save 2 mo</span></button></div></div><div className="mt-12 grid gap-4 lg:grid-cols-4">{plans.map((plan) => <article key={plan.name} className={`flex flex-col rounded-2xl border p-6 ${plan.featured ? 'border-[#b2761b] bg-[#fbf0dc]' : 'border-[#dfe5dc] bg-[#f7f7f3]'}`}><div className="flex items-center justify-between"><h3 className="text-lg font-semibold text-[#173b2b]">{plan.name}</h3>{plan.featured && <span className="rounded-full bg-[#b2761b] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-white">Popular</span>}</div><p className="mt-6 text-2xl font-semibold text-[#173b2b]">{formatPrice(plan.monthly, annual)}<span className="text-sm font-normal text-[#829086]"> / month</span></p><p className="mt-2 text-sm text-[#65746a]">{plan.limit}</p><p className="mt-6 min-h-12 text-sm leading-6 text-[#65746a]">{plan.description}</p><Link href="/start-free-trial" className="mt-6 inline-flex items-center text-sm font-semibold text-[#173b2b]">Get started <ArrowRight className="ml-2 h-4 w-4" /></Link></article>)}</div></section>

      <section className="mx-auto max-w-4xl px-5 pb-20 lg:px-8"><p className="text-xs font-bold uppercase tracking-[0.22em] text-[#b2761b]">Questions, answered</p><h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-[#173b2b]">A clearer start for your team.</h2><div className="mt-8 divide-y divide-[#dfe5dc] border-y border-[#dfe5dc]">{faqs.map(([question, answer]) => <details key={question} className="group py-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-6 text-base font-semibold text-[#304237] [&::-webkit-details-marker]:hidden">{question}<ChevronDown className="h-5 w-5 shrink-0 text-[#b2761b] transition-transform group-open:rotate-180" /></summary><p className="max-w-2xl pt-3 leading-7 text-[#65746a]">{answer}</p></details>)}</div></section>
